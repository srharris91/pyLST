...
class MyWindow:
    def __init__(self, win):
        ...
        lalpha = tk.ttk.Label(self.fLST1,text=u'\u03B1'); lalpha.grid(  row=1,column=0)
        self.alpha = tk.ttk.Entry(self.fLST1,width=10); self.alpha.grid(row=1,column=1)
        ...
        lomega = tk.ttk.Label(self.fLST1,text=u'\u03C9 of id'); lomega.grid(row=15,column=0)
        self.lomega = tk.ttk.Label(self.fLST1,text=''); self.lomega.grid(row=15,column=1)
        ...
        self.ax_eigenvalues.set_xlabel(r'$\omega_r$')
        self.ax_eigenvalues.set_ylabel(r'$\omega_i$')
        ...
        self.fig_eigenfunction = plt.Figure(figsize=(7.3, 5.5),tight_layout=True)#, dpi=100)
        self.ax_ubar = self.fig_eigenfunction.add_subplot(251)
        self.ax_uhat = self.fig_eigenfunction.add_subplot(252,sharey=self.ax_ubar)
        self.ax_vhat = self.fig_eigenfunction.add_subplot(253,sharey=self.ax_ubar)
        self.ax_what = self.fig_eigenfunction.add_subplot(254,sharey=self.ax_ubar)
        self.ax_phat = self.fig_eigenfunction.add_subplot(255,sharey=self.ax_ubar)
        self.ax_uhatdagger = self.fig_eigenfunction.add_subplot(257,sharey=self.ax_ubar)
        self.ax_vhatdagger = self.fig_eigenfunction.add_subplot(258,sharey=self.ax_ubar)
        self.ax_whatdagger = self.fig_eigenfunction.add_subplot(259,sharey=self.ax_ubar)
        self.ax_phatdagger = self.fig_eigenfunction.add_subplot(2,5,10,sharey=self.ax_ubar)
        ...
        self.ax_uhatdagger.tick_params(labelleft=False)
        self.ax_vhatdagger.tick_params(labelleft=False)
        self.ax_whatdagger.tick_params(labelleft=False)
        self.ax_phatdagger.tick_params(labelleft=False)
        ...
        self.ax_uhatdagger.set_xlabel(r'$\hat{u}^\dagger$')
        self.ax_vhatdagger.set_xlabel(r'$\hat{v}^\dagger$')
        self.ax_whatdagger.set_xlabel(r'$\hat{w}^\dagger$')
        self.ax_phatdagger.set_xlabel(r'$\hat{p}^\dagger$')
        ...
        self.uhatdaggerline1, = self.ax_uhatdagger.plot(self.t,self.t,'-',label='real')
        self.uhatdaggerline2, = self.ax_uhatdagger.plot(self.t,self.t,'--',label='imag')
        self.uhatdaggerline3, = self.ax_uhatdagger.plot(self.t,self.t,'k-',label='abs')
        self.vhatdaggerline1, = self.ax_vhatdagger.plot(self.t,self.t,'-',label='real')
        self.vhatdaggerline2, = self.ax_vhatdagger.plot(self.t,self.t,'--',label='imag')
        self.vhatdaggerline3, = self.ax_vhatdagger.plot(self.t,self.t,'k-',label='abs')
        self.whatdaggerline1, = self.ax_whatdagger.plot(self.t,self.t,'-',label='real')
        self.whatdaggerline2, = self.ax_whatdagger.plot(self.t,self.t,'--',label='imag')
        self.whatdaggerline3, = self.ax_whatdagger.plot(self.t,self.t,'k-',label='abs')
        self.phatdaggerline1, = self.ax_phatdagger.plot(self.t,self.t,'-',label='real')
        self.phatdaggerline2, = self.ax_phatdagger.plot(self.t,self.t,'--',label='imag')
        self.phatdaggerline3, = self.ax_phatdagger.plot(self.t,self.t,'k-',label='abs')
    ...
    def _plot_spectrum(self):
        ...
        self.eigenvalues_line.set_label(r'$\omega$')
    ...
    def _plot_id(self):
        ...
        self.qhat0dagger = self.eigenfuncl[:4*ny,ind]
        self.omega0= self.eigenvalues[ind]
        uhat = self.qhat0[    :1*ny]
        vhat = self.qhat0[1*ny:2*ny]
        what = self.qhat0[2*ny:3*ny]
        phat = self.qhat0[3*ny:    ]
        uhatdagger = self.qhat0dagger[    :1*ny]
        vhatdagger = self.qhat0dagger[1*ny:2*ny]
        whatdagger = self.qhat0dagger[2*ny:3*ny]
        phatdagger = self.qhat0dagger[3*ny:    ]
        # plot data
        ...
        # plot adjoint
        self.uhatdaggerline1.set_xdata(uhatdagger.real)
        self.uhatdaggerline1.set_ydata(y)
        self.uhatdaggerline2.set_xdata(uhatdagger.imag)
        self.uhatdaggerline2.set_ydata(y)
        self.uhatdaggerline3.set_xdata(np.abs(uhatdagger))
        self.uhatdaggerline3.set_ydata(y)
        self.vhatdaggerline1.set_xdata(vhatdagger.real)
        self.vhatdaggerline1.set_ydata(y)
        self.vhatdaggerline2.set_xdata(vhatdagger.imag)
        self.vhatdaggerline2.set_ydata(y)
        self.vhatdaggerline3.set_xdata(np.abs(vhatdagger))
        self.vhatdaggerline3.set_ydata(y)
        self.whatdaggerline1.set_xdata(whatdagger.real)
        self.whatdaggerline1.set_ydata(y)
        self.whatdaggerline2.set_xdata(whatdagger.imag)
        self.whatdaggerline2.set_ydata(y)
        self.whatdaggerline3.set_xdata(np.abs(whatdagger))
        self.whatdaggerline3.set_ydata(y)
        self.phatdaggerline1.set_xdata(phatdagger.real)
        self.phatdaggerline1.set_ydata(y)
        self.phatdaggerline2.set_xdata(phatdagger.imag)
        self.phatdaggerline2.set_ydata(y)
        self.phatdaggerline3.set_xdata(np.abs(phatdagger))
        self.phatdaggerline3.set_ydata(y)

        #self.eigenvalues_line.set_ydata(2 * np.sin(2 * np.pi * self.t))
        #self.eigenvalues_line.set_label(r'$2\sin{(2 \pi t)}$')
        #self.ax_eigenvalues.legend(loc='best',numpoints=1)
        for ax in [self.ax_uhat,self.ax_vhat,self.ax_what,self.ax_phat, self.ax_uhatdagger,self.ax_vhatdagger,self.ax_whatdagger,self.ax_phatdagger]:
    ...
    def _solve(self):
        ...
        if self.full_partial_var.get()=='full':
            ...
            self.L,self.M,self.eigenvalues,self.eigenfuncl,self.eigenfuncr = LST_temporal(self.params,self.diffs,self.baseflow,self.helper_mats)
        #elif self.full_partial_var.get()=='single':
            #print('solve value is ',self.full_partial_var.get())
            #omegas=[complex(self.eigenvalue_guess.get().replace(' ','')),]
            #self.L,self.M,self.eigenvalues,self.eigenfuncl,self.eigenfuncr = LST_temporal(self.params,self.diffs,self.baseflow,self.helper_mats,omegas)
        else:
            print('SOMETHING IS WRONG!!! No single solve available for temporal. solve value is ',self.full_partial_var.get())
    ...
    def onpick(self,event):
        ...
        self.lomega['text']='{:.2e}+{:.2e}i'.format(*points)
    ...
    def _open(self):
        ...
        if (self.file!='') and ('.h5' in self.file):
            ...
            if 'LST' in f:
                ...
                self.omega0 = self._read_var_in_h5(gLST,'eigenfuncr_alpha')
