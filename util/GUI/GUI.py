...
from ..base_flow import blasius,channel,tanh
from ..Classes import grid1D_omega_beta_class
from ..Classes import base_flow_class
from ..grid import set_D,set_D_Chebyshev,set_Cheby_mapped_y,set_FD_stretched_y
...
class MyWindow:
    def __init__(self, win):
        self.win=win
        self.win.wm_iconphoto(True,tk.PhotoImage(file='./util/GUI/pyLST.png'))
        self.win.title("pyLST GUI")
        ...
        self.menu.config(background=self.win['background'])
        ...
        self.File_items.config(background=self.win['background'])
        ...
        self.tabLST = tk.ttk.Frame(self.tab_control)
        self.tabLPSE = tk.ttk.Frame(self.tab_control)
        self.tabNPSE = tk.ttk.Frame(self.tab_control)
        self.tabSPE = tk.ttk.Frame(self.tab_control)
        #self.tab3 = tk.ttk.Frame(self.tab_control)
        self.tab_control.add(self.tabLST, text='LST')
        self.tab_control.add(self.tabLPSE, text='LPSE')
        self.tab_control.add(self.tabNPSE, text='NPSE')
        self.tab_control.add(self.tabSPE, text='SPE')
        ...
        # LST items
        # add radio buttons
        self.fLST1 = tk.ttk.Frame(self.tabLST)
        row=0
        ...
        self.lbl1.grid(row=0,column=0,columnspan=3); row+=1
        # dropdown for discretization and LST type
        disc_op = tk.ttk.Label(self.fLST1,text='Grid operators'); disc_op.grid(            row=row,column=0)
        disc_op_guess = tk.ttk.Label(self.fLST1,text='Finite Diff. stretching ratio'); disc_op_guess.grid(               row=row,column=1); row+=1
        self.disc_op_var,self.disc_op_dim = self.set_dropdown(self.fLST1,['Finite Diff','Chebyshev'],row=row,column=0)
        self.disc_op_guess = tk.ttk.Entry(self.fLST1,width=10); self.disc_op_guess.grid(     row=row,column=1); row+=1
        # dropdown for baseflow type
        baseflow_type = tk.ttk.Label(self.fLST1,text='Baseflow type'); baseflow_type.grid(            row=row,column=0); row+=1
        self.baseflow_type_var,self.baseflow_type_dim = self.set_dropdown(self.fLST1,['Blasius BL','Channel', 'Tanh','User Defined'],row=row,column=0)
        self.baseflow_type_var.trace("w",self.trace_baseflow_type_var)
        row +=1
        # other flow parameters
        #lomega = tk.ttk.Label(self.f1,text=u'\u03C9 or \u03B1')
        lomega = tk.ttk.Label(self.fLST1,text=u'\u03C9'); lomega.grid(  row=row,column=0)
        self.omega = tk.ttk.Entry(self.fLST1,width=10); self.omega.grid(row=row,column=1); row+=1
        lbeta = tk.ttk.Label(self.fLST1,text=u'\u03B2'); lbeta.grid(    row=row,column=0)
        self.beta = tk.ttk.Entry(self.fLST1,width=10); self.beta.grid(  row=row,column=1); row+=1
        lRe = tk.ttk.Label(self.fLST1,text='Re'); lRe.grid(             row=row,column=0)
        self.Re = tk.ttk.Entry(self.fLST1,width=10); self.Re.grid(      row=row,column=1); row+=1
        lny = tk.ttk.Label(self.fLST1,text='ny'); lny.grid(             row=row,column=0)
        self.ny = tk.ttk.Entry(self.fLST1,width=10); self.ny.grid(      row=row,column=1); row+=1
        ly = tk.ttk.Label(self.fLST1,text='y range'); ly.grid(          row=row,column=0)
        self.y0 = tk.ttk.Entry(self.fLST1,width=10); self.y0.grid(      row=row,column=1)
        self.y1 = tk.ttk.Entry(self.fLST1,width=10); self.y1.grid(      row=row,column=2); row+=1
        self.button1_solve_baseflow = tk.ttk.Button(master=self.fLST1, text="Solve and Plot Baseflow", command=self._solve_baseflow)
        self.button1_solve_baseflow.grid(row=row,column=0); row+=1
        # dropdown menus
        ...
        # eigenvalue spectrum (full or single mode)
        leigenvalue = tk.ttk.Label(self.fLST1,text='Spectrum Soln.'); leigenvalue.grid(            row=row,column=0)
        leigenvalue_guess = tk.ttk.Label(self.fLST1,text='Guess'); leigenvalue_guess.grid(               row=row,column=1); row+=1
        self.full_partial_var,self.full_partial_dim = self.set_dropdown(self.fLST1,['full','single'],row=row,column=0)
        self.eigenvalue_guess = tk.ttk.Entry(self.fLST1,width=10); self.eigenvalue_guess.grid(     row=row,column=1); row+=1
        ...
        self.button1_solve.grid(row=row,column=0)
        self.button2_plot = tk.ttk.Button(master=self.fLST1, text="Plot", command=self._plot_spectrum)
        self.button2_plot.grid(row=row,column=1); row+=1
        self.button3_save = tk.ttk.Button(master=self.fLST1, text="Save", command=self._save_spectrum)#, command=self._solve)
        self.button3_save.grid(row=row,column=0)
        self.button4_save = tk.ttk.Button(master=self.fLST1, text="Zoom", command=self._zoom_plot_spectrum)#, command=self._solve)
        self.button4_save.grid(row=row,column=1); row+=1
        # ID and eigenfunction plots
        lid = tk.ttk.Label(self.fLST1,text='id'); lid.grid(        row=row,column=0)
        self.lid = tk.ttk.Label(self.fLST1,text=''); self.lid.grid(row=row,column=1); row+=1
        # add plot and save to eigenfunction
        self.button4_plot = tk.ttk.Button(master=self.fLST1, text="Plot id", command=self._plot_id)
        self.button4_plot.grid(row=row,column=0)
        self.button5_save = tk.ttk.Button(master=self.fLST1, text="Save id", command=self._save_id)
        self.button5_save.grid(row=row,column=1); row+=1
        # add eigenvalue spot
        #lalpha = tk.ttk.Label(self.f1,text=u'\u03C9 or \u03B1 of id'); lalpha.grid(row=14,column=0)
        lalpha = tk.ttk.Label(self.fLST1,text=u'\u03B1 of id'); lalpha.grid(row=row,column=0)
        self.lalpha = tk.ttk.Label(self.fLST1,text=''); self.lalpha.grid(row=row,column=1); row+=1
        ...
        self.fig_baseflow = plt.Figure(figsize=(7.3, 2.5),tight_layout=True)#, dpi=100)
        self.ax_ubar = self.fig_baseflow.add_subplot(131)
        self.ax_vbar = self.fig_baseflow.add_subplot(132,sharey=self.ax_ubar)
        self.ax_pbar = self.fig_baseflow.add_subplot(133,sharey=self.ax_ubar)
        self.ax_vbar.tick_params(labelleft=False)
        self.ax_pbar.tick_params(labelleft=False)
        self.ax_ubar.set_ylabel(r'$y$')
        self.ax_ubar.set_xlabel(r'$\overline{u}$')
        self.ax_vbar.set_xlabel(r'$\overline{v}$')
        self.ax_pbar.set_xlabel(r'$\overline{p}$')
        self.ubar_line, = self.ax_ubar.plot(self.t,self.t,'-') # plot zeros on ubar
        self.vbar_line, = self.ax_vbar.plot(self.t,self.t,'-')
        self.pbar_line, = self.ax_pbar.plot(self.t,self.t,'-')
        self.fLST3 = tk.ttk.Frame(self.tabLST)
        #self.fLST3.pack(fill=tk.BOTH, expand=1)#side=tk.LEFT)
        self.fLST3.grid(row=0,column=1)
        self.canvas_baseflow = FigureCanvasTkAgg(self.fig_baseflow, master=self.fLST3)  # A tk.DrawingArea.
        ...
        #self.toolbar_frame_eigenfunction = tk.ttk.Frame(self.tabLST)
        #self.fLST2.pack(fill=tk.BOTH, expand=1)#side=tk.LEFT)
        self.toolbar_baseflow = NavigationToolbar2Tk(self.canvas_baseflow, self.fLST3)
        self.toolbar_baseflow.update()
        self.canvas_baseflow.get_tk_widget().pack()#side=tk.BOTTOM)#, fill=tk.BOTH, expand=1)
        self.toolbar_baseflow.config(background=self.win['background'])
        #self.toolbar_baseflow.pack()
        #self.canvas_eigenfunction.mpl_connect("key_press_event", self.on_key_press_eigenfunction)
        # plot figure
        self.fig_eigenvalues = plt.Figure(figsize=(7.3, 2.5),tight_layout=True)#, dpi=100)
        self.ax_eigenvalues = self.fig_eigenvalues.add_subplot(111)
        self.eigenvalues_line, = self.ax_eigenvalues.plot(self.t,self.t,'o',picker=True)
            #self.t, 
        ...
        self.eigenvalues_line.set_pickradius(10.0)
        #self.plot_sin()
        # export figure to canvas_eigenvalues widget
        self.fLST2 = tk.ttk.Frame(self.tabLST)
        #self.fLST2.pack(fill=tk.BOTH, expand=1)#side=tk.LEFT)
        self.fLST2.grid(row=1,column=1)
        ...
        # get NavigationToolbar widget
        #self.toolbar_frame_eigenvalues = tk.ttk.Frame(self.tabLST)
        ...
        self.toolbar_eigenvalues.config(background=self.win['background'])
        self.canvas_eigenvalues.get_tk_widget().pack()#side=tk.BOTTOM)#, fill=tk.BOTH, expand=1)
        # trace key presses
        ...
        self.fig_eigenfunction = plt.Figure(figsize=(7.3, 2.5),tight_layout=True)#, dpi=100)
        self.ax_ubar2 = self.fig_eigenfunction.add_subplot(151)
        self.ax_uhat = self.fig_eigenfunction.add_subplot(152,sharey=self.ax_ubar2)
        self.ax_vhat = self.fig_eigenfunction.add_subplot(153,sharey=self.ax_ubar2)
        self.ax_what = self.fig_eigenfunction.add_subplot(154,sharey=self.ax_ubar2)
        self.ax_phat = self.fig_eigenfunction.add_subplot(155,sharey=self.ax_ubar2)
        ...
        self.ax_ubar2.set_ylabel(r'$y$')
        self.ax_ubar2.set_xlabel(r'$\overline{u}$')
        ...
        self.ubar_line2, = self.ax_ubar2.plot(self.t,self.t,'-') # plot zeros on ubar
        ...
        self.fLST4 = tk.ttk.Frame(self.tabLST)
        #self.fLST4.pack(fill=tk.BOTH, expand=1)#side=tk.LEFT)
        self.fLST4.grid(row=2,column=1)
        self.canvas_eigenfunction = FigureCanvasTkAgg(self.fig_eigenfunction, master=self.fLST4)  # A tk.DrawingArea.
        #self.canvas_eigenfunction.draw()
        #self.canvas_eigenfunction.get_tk_widget().pack()#side=tk.BOTTOM)#, fill=tk.BOTH, expand=1)
        # get NavigationToolbar widget
        #self.toolbar_frame_eigenfunction = tk.ttk.Frame(self.tabLST)
        #self.fLST2.pack(fill=tk.BOTH, expand=1)#side=tk.LEFT)
        self.toolbar_eigenfunction = NavigationToolbar2Tk(self.canvas_eigenfunction, self.fLST4)
        self.toolbar_eigenfunction.update()
        self.toolbar_eigenfunction.config(background=self.win['background'])
        self.canvas_eigenfunction.get_tk_widget().pack()#side=tk.BOTTOM)#, fill=tk.BOTH, expand=1)
        #self.canvas_eigenfunction.mpl_connect("key_press_event", self.on_key_press_eigenfunction)

        # LPSE
        self.fPSE1 = tk.ttk.Frame(self.tabLPSE)
        ...
        self.fPSE2 = tk.ttk.Frame(self.tabLPSE)
        ...
        self.toolbar_PSE.config(background=self.win['background'])
        ...
        #self.fPSE3 = tk.ttk.Frame(self.tabLPSE)
        ...
        self.button_save_pse = tk.ttk.Button(master=self.fPSE1, text="Save", command=self._save); self.button_save_pse.grid(                    row=10,column=2)
        self.fPSE1.grid(row=0,column=0)
    ...
    def trace_baseflow_type_var(self,*args):
        baseflow_type = self.baseflow_type_var.get()
        if baseflow_type == 'Blasius BL':
            self.set_text(self.y0,'{:g}'.format(0.0))
            self.set_text(self.y1,'{:g}'.format(61.0))
        elif baseflow_type == 'Channel':
            self.set_text(self.y0,'{:g}'.format(-1.0))
            self.set_text(self.y1,'{:g}'.format(1.0))
        elif baseflow_type == 'Tanh':
            self.set_text(self.y0,'{:g}'.format(-10.0))
            self.set_text(self.y1,'{:g}'.format(10.0))
        elif baseflow_type == 'User Defined':
            self.set_text(self.y0,'')
            self.set_text(self.y1,'')

        print(self.baseflow_type_var.get(), 'selected')
    ...
    def _plot_spectrum(self):
        ...
        self.ax_eigenvalues.set_ylabel(r'$\alpha_i$')
        # plot ubar
        self.ubar_line2.set_xdata(self.baseflow.get_U())
        self.ubar_line2.set_ydata(self.params['grid'].y)
        ...
        #self.ubar_line2, = self.ax_ubar.plot(self.baseflow.get_U(),self.params['grid'].y)
        self.fig_eigenvalues.canvas.draw()
        #self.fig.tight_layout()

    def _zoom_plot_spectrum(self):
    ...
    def _solve_baseflow(self):
        # get values from fields
        ny = int(self.ny.get())
        Re = float(self.Re.get())
        y0 = float(self.y0.get())
        y1 = float(self.y1.get())
        # create grid and solve LST for boundary layer
        if self.baseflow_type_var.get()=='Blasius BL':
            nu=1./Re
            x_start = Re
            if self.disc_op_var.get()=='Finite Diff':
                delta=float(self.disc_op_guess.get().replace(' ',''))
                if y0==0.0:
                    y = set_FD_stretched_y(y1,ny,delta=delta)
                else:
                    y = np.linspace(y0,y1,ny)
            elif self.disc_op_var.get()=='Chebyshev':
                y = set_Cheby_mapped_y(y0,y1,ny)
        elif self.baseflow_type_var.get()=='Channel':
            nu = 1./Re
            x_start = 0
            if self.disc_op_var.get()=='Finite Diff':
                y = np.linspace(y0,y1,ny) # TODO smarter grid stretching?
            elif self.disc_op_var.get()=='Chebyshev':
                y = set_Cheby_mapped_y(y0,y1,ny)
        elif self.baseflow_type_var.get()=='Tanh':
            nu = 1./Re
            x_start=0
            if self.disc_op_var.get()=='Finite Diff':
                y = np.linspace(y0,y1,ny) # TODO smarter grid spacing?
            elif self.disc_op_var.get()=='Chebyshev':
                y = set_Cheby_mapped_y(y0,y1,ny)
        else:
            raise ValueError('baseflow_type_var is not Blasius BL')
        self.y = y
        ...
        if self.disc_op_var.get()=='Finite Diff':
            ...
            self.diffs={
                    'Dy':set_D_Chebyshev(y,d=1,need_map=True),
                    'Dyy':set_D_Chebyshev(y,d=2,need_map=True)
                    }
        # get baseflow
        if self.baseflow_type_var.get()=='Blasius BL':
            self.baseflow = blasius(y,x=x_start,nu=nu,plot=False)
        elif self.baseflow_type_var.get()=='Channel':
            self.baseflow = channel(y,x=x_start,nu=nu,plot=False)
        elif self.baseflow_type_var.get()=='Tanh':
            self.baseflow = tanh(y,self.diffs,plot=False)
        elif self.baseflow_type_var.get()=='User Defined':
            raise ValueError("User Defined baseflow not implemented yet")

        # plot baseflow
        # u-velocity
        self.ubar_line.set_xdata(self.baseflow.get_U())
        self.ubar_line.set_ydata(self.y)
        self.ax_ubar.relim() # recompute the ax.dataLim
        self.ax_ubar.autoscale_view() # update ax.viewLim using the new dataLim
        self.ubar_line2.set_xdata(self.baseflow.get_U())
        self.ubar_line2.set_ydata(self.y)
        self.ax_ubar2.relim() # recompute the ax.dataLim
        self.ax_ubar2.autoscale_view() # update ax.viewLim using the new dataLim
        self.fig_eigenfunction.canvas.draw() # update figure to view changes
        # v-velocity
        self.vbar_line.set_xdata(self.baseflow.get_V())
        self.vbar_line.set_ydata(self.y)
        self.ax_vbar.relim() # recompute the ax.dataLim
        self.ax_vbar.autoscale_view() # update ax.viewLim using the new dataLim
        # p-pressure
        self.pbar_line.set_xdata(self.baseflow.get_P())
        self.pbar_line.set_ydata(self.y)
        self.ax_pbar.relim() # recompute the ax.dataLim
        self.ax_pbar.autoscale_view() # update ax.viewLim using the new dataLim
        # refresh figure
        self.fig_baseflow.canvas.draw()
    ...
    def _solve(self):
        ...
        self.params={
                ...
                'omega':omega,
                'beta':beta,
                'grid':grid1D_omega_beta_class(y=self.y),
                'ny':self.y.shape[0],
                'x_start':x_start,
                'x':x_start,
                'nu':nu,
...
def GUI():
    #window=tk.Tk(className='pyLST')
    window=ttkthemes.ThemedTk(theme='clearlooks',background=True,className=r"pyLST")
